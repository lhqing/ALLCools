...
def _annotate_by_beds_worker(dataset_path, region_dim, chrom_size_path,
                             slop, track_paths, dtype, dim, output_path, bed_sorted, **kwargs):
    len(kwargs)
...
class RegionDS(xr.Dataset):
    ...
    def __init__(self, dataset, region_dim=None, location=None,
    ...
    def from_bed(cls, bed, location, chrom_size_path, region_dim='region'):
        ...
        if n_cols == 3:
            ...
            raise ValueError('bed file need to be either 3 columns (chrom, start, end) '
            ...
            if _path.is_dir():
                ...
                if region_ds_file.exists():
                    with open(region_ds_file) as f:
                        ...
                        exclude_dir_name = [k for k, v in region_ds_config['ds_region_dim'].items()
                                            if v != region_dim]
                    ...
                    for sub_dir_path in _path.iterdir():
                        if sub_dir_path.is_dir() and sub_dir_path.name[0] != '.':
                            ...
                            try:
                                ...
                                raise e
                    region_ds = cls(xr.merge(datasets),
                                    region_dim=region_dim,
        ...
        if region_state_da is None:
            ...
            if use_collapsed:
                ...
                if region_state_da not in self.data_vars:
                    print(f'use_collapsed=True but the "{region_dim}_state_collapsed" '
