...
class ClusterMerge:
    ...
    def _construct_tree(self):
        cells = (
            self.cell_to_type.to_frame()
            .groupby(self.cell_to_type.name)
            .apply(lambda x: x.sample(self.n_cells, replace=True))
            .droplevel(0)
            .sort_index()
            .index
        )
    ...
    def _merge_pair(self, pair, concat_str="::"):
        ...
        separable, evidence, *_ = self.merge_criterion.predict(
            (left_lbl, right_lbl), pair_cells, pair_cell_types, pair_mcds
        )
    ...
    def fit_predict(self, data_for_tree, cell_to_type, gene_mcds):
        ...
        while True:
            ...
            if self.stop_criterion is not None and self.stop_criterion(
                self.data_for_tree, self.cell_to_type, self.gene_mcds
            ):
                print()  # TODO
                break
...
class PairwiseDMGCriterion:
    ...
    def __init__(
        ...
        self.pwdmg = PairwiseDMG(
            max_cell_per_group=max_cell_per_group,
            top_n=top_n_markers,
    ...
    def predict(self, pair_labels, pair_cells, pair_cell_types, pair_mcds, da_name="gene_da_frac"):
        ...
        for mc_type in mc_types:
            self.pwdmg.fit_predict(x=pair_mcds[da_name].sel(mc_type=mc_type), groups=pair_cell_types)
