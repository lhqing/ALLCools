...
def _create_train_region_ds(reptile):
    # train regions
    train_regions_bed = pybedtools.BedTool(
        reptile.train_regions).sort(g=reptile.chrom_size_path)

    # train region labels
    train_label = pd.read_csv(reptile.train_region_labels,
                              sep='\t',
                              index_col=0,
                              squeeze=True)
...
def _create_train_dmr_ds(reptile, train_regions_bed, train_label):
    # total DMRs
    dmr_regions_bed = pybedtools.BedTool(
        reptile.dmr_regions).sort(g=reptile.chrom_size_path)
    dmr_regions_bed_df = dmr_regions_bed.to_dataframe()

    # train DMRs and train DMR labels
    train_dmr = train_regions_bed.map(dmr_regions_bed, c=4,
                                      o='collapse').to_dataframe()
    ...
    train_dmr_regions_bed_df = dmr_regions_bed_df.set_index('name').loc[
                                   dmr_label.index].reset_index().iloc[:, [1, 2, 3, 0]]
...
def _create_query_region_ds(reptile):
    pybedtools.BedTool().makewindows(
        ...
        s=reptile.step_size,
        w=reptile.window_size).saveas(f"{reptile.output_path}/query_region.bed")
...
def _predict_sample(region_ds_path,
    ...
    with dask.config.set(scheduler='sync'):
        ...
        for chunk_start in range(0, region_ids.size, chunk_size):
            ...
            data, _ = _get_data_and_label(region_ds=_region_ds,
...
class REPTILE:
    def __init__(self,
        ...
        if isinstance(bigwig_table, (str, pathlib.PosixPath)):
            ...
            if bigwig_table.endswith('tsv'):
                ...
                sep = ','
            self.bigwig_table = pd.read_csv(bigwig_table, sep=sep,
                                            index_col=0)
        else:
    ...
    def train_region_ds(self):
        if self._train_region_ds is None:
            self._train_region_ds = RegionDS.open(self.output_path,
                                                  region_dim='train-region',
                                                  engine='zarr')
    ...
    def train_dmr_ds(self):
        if self._train_dmr_ds is None:
            self._train_dmr_ds = RegionDS.open(self.output_path,
                                               region_dim='train-dmr',
                                               engine='zarr')
    ...
    def query_region_ds(self):
        if self._query_region_ds is None:
            self._query_region_ds = RegionDS.open(self.output_path,
                                                  region_dim='query-region',
                                                  engine='zarr')
    ...
    def annotate_by_bigwigs(self, name, slop, cpu, redo=False):
        ...
        for modality, bigwig_paths in self.bigwig_table.items():
            ...
            with warnings.catch_warnings():
                ...
                region_ds.coords[f'{modality}_mean'] = region_ds[da_name].sel(
                    {modality: self.samples}).mean(dim=modality, skipna=True).load()
                region_ds.coords[f'{modality}_std'] = region_ds[da_name].sel(
                    {modality: self.samples}).std(dim=modality, skipna=True).load()
                region_ds.coords[f'{modality}_nan_count'] = np.isnan(
                    region_ds[da_name].sel({modality: self.samples})).sum(
                    dim=modality).load()
    ...
    def _filter_na_train(self, name, sample, max_na_rate=0.5):
        ...
        for modality in self.modalities:
            if modality not in region_ds.dims:
                raise KeyError(f'{name} RegionDS missing {modality} annotation, '
    ...
    def prepare_training_input(self, name):
        ...
        data, label = _get_data_and_label(region_ds=region_ds,
    ...
    def auto_ml(data,
        ...
        tpot = TPOTClassifier(generations=tpot_generations,
    ...
    def _predict(self, region_dim, cpu, mask_cutoff):
        ...
        with ProcessPoolExecutor(cpu) as exe:
            ...
            for chunk_i, sample in enumerate(self.samples):
                ...
                future = exe.submit(_predict_sample,
        ...
        update_region_ds_config(output_dir=self.output_path,
    ...
    def _dump_sample(self, sample, mask_cutoff, bw_bin_size):
        ...
        with dask.config.set(scheduler='sync'):
            ...
            dmr_value = dmr_pred.get_feature(sample,
                                             dim='sample',
                                             da_name='query-dmr_prediction')
            ...
            dmr_bed_df.to_csv(f'{self.bigwig_dir}/{sample}_dmr_pred.bg',
                              sep='\t',
                              index=None,
                              header=None)
            ...
            region_value = region_pred.get_feature(sample,
                                                   dim='sample',
                                                   da_name='query-region_prediction')
            ...
            region_bed_df.to_csv(f'{self.bigwig_dir}/{sample}_region_pred.bg',
                                 sep='\t',
                                 index=None,
                                 header=None)
            ...
            with pyBigWig.open(bw_path, 'w') as bw:
                chrom_sizes = pd.read_csv(self.chrom_size_path,
                                          sep='\t',
                                          index_col=0,
                                          header=None,
                                          squeeze=True).to_dict()
                bw.addHeader([(k, v) for k, v in pd.Series(chrom_sizes).sort_index().items()])
            ...
            subprocess.run(f'rm -f {self.bigwig_dir}/{sample}_dmr_pred.bg '
                           f'{self.bigwig_dir}/{sample}_region_pred.bg',
                           shell=True)
        return bw_path
    ...
    def dump_bigwigs(self, cpu, mask_cutoff, bw_bin_size):
        ...
        with ProcessPoolExecutor(cpu) as exe:
            ...
            for sample in self.samples:
                f = exe.submit(self._dump_sample,
                               sample=sample,
                               mask_cutoff=mask_cutoff,
