...
def _read_single_allc(path, region):
    ...
    data = pd.DataFrame(
        rows,
        columns=['chrom', 'pos', 'strand', 'context', 'mc', 'cov']).set_index('pos')
...
def _make_count_table(path_list, region):
    ...
    total_counts = pd.concat(
        [df.reindex(all_contexts.index) for df in all_counts],
        axis=1).fillna(0)
...
def _perform_rms_batch(output_dir, allc_paths, samples, region, max_row_count, n_permute, min_pvalue):
    contexts, total_counts = _make_count_table(path_list=allc_paths,
                                               region=region)
    ...
    for pos, row in total_counts.iterrows():
        ...
        p = permute_root_mean_square_test(_table,
                                          n_permute=n_permute,
    ...
    da = xr.DataArray(site_matrix[:, :, :2],
                      coords=[samples, pos_order, ['mc', 'cov']],
                      dims=['sample', 'pos', 'count_type'])
    residual_da = xr.DataArray(site_matrix[:, :, 2],
                               coords=[samples, pos_order],
                               dims=['sample', 'pos'])
    ...
    da.coords['chrom'] = pd.Series(region.split(':')[0],
                                   index=da.get_index('pos'))
...
def call_dms(output_dir, allc_paths, samples, chrom_size_path, cpu=1,
    ...
    with open(f'{output_dir}/.ALLCools', 'w') as f:
        config = {'region_dim': 'dms',
                  'ds_region_dim': {'dms': 'dms'}}
        yaml.dump(config, f)

    subprocess.run(f'cp {chrom_size_path} {output_dir}/chrom_sizes.txt',
