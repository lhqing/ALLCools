...
def _read_snap_genes(file_path):
    ...
    with h5py.File(file_path) as f:
        ...
        data = ss.coo_matrix((data, (idx, idy)),
                             shape=(cell_barcode.size, gene_id.size),
                             dtype=np.uint8).tocsc()
...
def _read_snap_bins(file_path, bin_size=5000):
    ...
    with h5py.File(file_path) as f:
        ...
        idx = np.array(
            f[f'/AM/{bin_size}/idx']) - 1  # R is 1 based, python is 0 based
        idy = np.array(
            f[f'/AM/{bin_size}/idy']) - 1  # R is 1 based, python is 0 based

        bin_chrom = np.array(f[f'/AM/{bin_size}/binChrom']).astype(str)
        bin_start = np.array(
            f[f'/AM/{bin_size}/binStart']) - 1  # 0 based bed format
        bin_end = np.array(
            f[f'/AM/{bin_size}/binStart']) - 1 + bin_size  # 0 based bed format
        bin_id = np.core.defchararray.add(
            np.core.defchararray.add(bin_chrom, '-'), bin_start.astype(str))

        cell_meta, cell_barcode = _read_snap_meta(f)
        data = ss.coo_matrix((data, (idx, idy)),
                             shape=(cell_barcode.size, bin_id.size),
                             dtype=np.uint8).tocsc()
