...
def _fimo_runner(
    ...
    with open(tmp_path, "w") as out:
        p = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="utf8")
        first = True
        while True:
    ...
    if not is_genome_fasta:
        ...
        final_df = pd.DataFrame(chrom_start.str.split(":").tolist(), columns=["sequence_name", "region_start"])
...
def _scan_motif_over_fasta(
    ...
    lookup_table = pd.DataFrame(
        motif_records, columns=["uid", "name", "motif_file_path", "output_file_path"]
    ).set_index("uid")
...
def _aggregate_motif_beds(bed_file_paths, output_path, cpu=1, sort_mem_gbs=1):
    ...
    subprocess.run(
        shlex.split(f"sort -k1,1 -k2,2n --parallel={cpu} -S {sort_mem_gbs}G {temp_bed} -o {output_path}"),
        capture_output=True,
        encoding="utf8",
        check=True,
    )
