...
def _count_single_cmotif_bin_on_multiple_allc(cmotif_dict_path, allc_paths, region, count_binary, context_to_pattern):
                    ...
                    if count_binary:  # only for single cell, either methylated or unmethylated
                        for (rf_strand, motif_id) in motifs:
                            if mc > 0 and mc != cov:
                                ...
                                continue
                            motif_mc_records[(rf_strand, context_to_pattern[context], motif_id)] += 0 if mc == 0 else 1
...
def allc_motif_scan(allc_table, output_path, mc_contexts, c_motif_dir, count_binary=True, cpu=1):
    """\
    ...
    with ProcessPoolExecutor(cpu) as executor:
        ...
        for _, (chrom, start, end, file_name) in lookup_table.iterrows():
            ...
            future = executor.submit(
                _count_single_cmotif_bin_on_multiple_allc,
