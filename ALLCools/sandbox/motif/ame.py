...
def _single_ame_runner(fasta_path, motif_paths, ame_kws_str=""):
    ...
    try:
        subprocess.run(shlex.split(cmd), check=True, capture_output=True, encoding="utf8")
...
def ame(
    ...
    if sample is not None and sample <= df.shape[0]:
        ...
        df.to_csv(output_dir / "INPUT_BED_SAMPLED.bed", sep="\t", index=None, header=None)
    ...
    ame_kws_str = ""
    _ame_kws = {"noseq": "", "verbose": 1, "control": control_fasta}
    ...
    with ProcessPoolExecutor(cpu) as pool:
        ...
        for chunk_id, chunk in enumerate(motif_path_chunks):
            ...
            future = pool.submit(
                _single_ame_runner, fasta_path=output_fasta_path, motif_paths=chunk, ame_kws_str=_ame_kws_str
            )
    ...
    for i in range(len(motif_path_chunks)):
        ...
        try:
            if has_header:
                chunk_df = pd.read_csv(ame_out_path, sep="\t", index_col=0, comment="#").reset_index(drop=True)
            else:
                chunk_df = pd.read_csv(ame_out_path, header=None, sep="\t", index_col=0, comment="#").reset_index(
                    drop=True
                )
                chunk_df.columns = [
                    "motif_DB",
    ...
    total_records["motif_DB"] = total_records["motif_DB"].apply(lambda x: x.split("/")[-1] if isinstance(x, str) else x)
